...
class WidgetBridge
    : private BlynkWidgetBase<BlynkType>
{
public:
    WidgetBridge(BlynkType& blinkEM,uint8_t vPin)
    ...
    void setAuthToken(const char* token) {
        ...
        cmd.add(BlynkWidgetBase<BlynkType>::mPin);
        cmd.add("i");
        cmd.add(token);
        BlynkWidgetBase<BlynkType>::blinkem.sendCmd(BLYNK_CMD_BRIDGE, 0, cmd.getBuffer(), cmd.getLength()-1);
    }
    ...
    void virtualWriteBinary(int pin, const void* buff, size_t len) {
        ...
        cmd.add("vw");
        cmd.add(pin);
        BlynkWidgetBase<BlynkType>::blinkem.sendCmd(BLYNK_CMD_BRIDGE, 0, cmd.getBuffer(), cmd.getLength(), buff, len);
    }
